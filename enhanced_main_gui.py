import sys
from PySide6.QtWidgets import QApplication, QGraphicsView, QGraphicsScene, QMainWindow, QToolBar
...
# Removed 'DetachedLigatureItem' as it's no longer used
...
class EnhancedMainWindow(QMainWindow):
    ...
    def setup_toolbar(self):
        ...
        select_action = QAction("Select", self); select_action.setCheckable(True); select_action.setChecked(True)
        select_action.triggered.connect(lambda: self.view.interaction_manager.set_mode(InteractionMode.SELECT))
        toolbar.addAction(select_action); mode_group.addAction(select_action)
        cut_action = QAction("Create Cut", self); cut_action.setCheckable(True)
        cut_action.triggered.connect(lambda: self.view.interaction_manager.set_mode(InteractionMode.CREATE_CUT))
        toolbar.addAction(cut_action); mode_group.addAction(cut_action)
        predicate_action = QAction("Create Predicate", self); predicate_action.setCheckable(True)
        predicate_action.triggered.connect(lambda: self.view.interaction_manager.set_mode(InteractionMode.CREATE_PREDICATE))
        toolbar.addAction(predicate_action); mode_group.addAction(predicate_action)
        ligature_action = QAction("Create Ligature", self); ligature_action.setCheckable(True)
        ligature_action.triggered.connect(lambda: self.view.interaction_manager.set_mode(InteractionMode.CREATE_LIGATURE))
        toolbar.addAction(ligature_action); mode_group.addAction(ligature_action)
    ...
    def populate_scene(self):
        ...
        self.scene.addItem(p_item); self.scene.addItem(cut_item); self.scene.addItem(q_item)
